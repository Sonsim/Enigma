...
def home():
    ...
    reflectors = [A, B, C]
    global ENIGMA
    ENIGMA = EnigmaMachine.EnigmaMachine(A, I,II,III, PB, KB)
    return render_template('index.html',A=A,B=B,C=C,I=I,II=II,III=III,IV=IV,V=V, ENIGMA=ENIGMA)

@app.route('/GetEnigma', methods=["POST", "GET"])
def GetEnigma():
    
    return jsonify(ENIGMA.to_dict())
